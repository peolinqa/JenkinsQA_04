...
public class Song99BottlesGenaShepelTest extends BaseTest {
    @Test
    public void testSongLyrics(){
        ...
        getDriver().findElement(
                By.xpath("//div[@id='navigation']/ul[@id='submenu']/li/a[@href='lyrics.html']")).click();

        String[] songTextArray = new String[100];

        for(int i = 0; i < songTextArray.length; i++){
            int index = i + 1;
            songTextArray[i] = getDriver().findElement(By.xpath("//div[@id='main']/p[" + index + "]")).getText();
        }
        String actualResult = "";
        for(int i = 0; i < songTextArray.length; i++){
            actualResult = actualResult + songTextArray[i];
        }
