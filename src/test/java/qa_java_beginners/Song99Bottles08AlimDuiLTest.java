...
public class Song99Bottles08AlimDuiLTest extends BaseTest {
    ...
    public void testSongLyricText() {
        ...
        String[] paragraphArr = new String[100];
        for (int i = 0; i < paragraphArr.length; i++) {
            int index = i + 1;
            paragraphArr[i] = getDriver()
                    .findElement(By.xpath("//div[@id='main']/p[" + index + "]"))
                    .getText();
        }
        String actualResult = "";
        for (int i = 0; i < paragraphArr.length; i++) {
            actualResult += paragraphArr[i];
        }
