...
@Ignore
public class Song99BottlesIrynaOkunTest extends BaseTest {
    ...
    public void testSongLyricsText() {
        ...
        getDriver().findElement(By.xpath("//ul[@id='submenu']/li/a[@href='lyrics.html']")).click();

        String[] pTexts = new String[100];
        for (int i = 0; i < 100; i++) {
            int index = i + 1;
            pTexts[i] = getDriver().findElement(By.xpath("//div[@id='main']/p[" + index + "]")).getText();
        }

        String actualResult = "";
        for (int i = 0; i < pTexts.length; i++) {
            actualResult += pTexts[i];
        }
